...
data TAMState = TAMState {
    ...
    tsLB   :: MTInt,	-- LB (Local Base): 0 <= tsLB <= tsSL
    tsST   :: MTInt,	-- ST (Stack Top): 0 <= tsST <= tsSL
    tsStk  :: [MTInt]	-- Stack: length tsStk <= stkSz
}


-- TAM stack limit. Naive impl., so a large stack would be very inefficient.
-- Invariant: 0 <= tsST <= stkSz <= maxBound :: MTInt
stkSz = 1000
...
runTAM :: Bool -> [TAMInst] -> IO ()
runTAM trace code = rtAux (TAMState {tsCode = array (0, cdSz - 1)
                                                    (zip [0..] code),
                                     tsCdSz = cdSz,
                                     tsPC   = 0,
                                     tsLB   = 0,
...
-- TODO! Think through this!
        ...
        PUTCHR ->
            case pop s of
                Right (n, s') -> putChar (toEnum n) >> continue s' 
                Left msg      -> abort msg
        GETINT -> do
    ...
    where
        ...
        readStkAddr a = readStk False (addrToIx a)

        readStkInd d =
            case stk of
                a : _ -> readStk True (addrToIx (SB (a + d)))
                _     -> abort stkUFlw
        ...
        writeStk :: Int -> MTInt -> MTInt -> IO (Maybe TAMState)
        writeStk n x d
            | 0 <= d && d < st =
